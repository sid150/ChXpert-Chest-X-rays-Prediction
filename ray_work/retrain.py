...
from lightning.pytorch.callbacks import ModelCheckpoint, EarlyStopping, BackboneFinetuning, Callback
import pynvml

import mlflow
from mlflow.tracking import MlflowClient                   # <<< RETRAIN SUPPORT >>>
import mlflow.pytorch                                      # <<< RETRAIN SUPPORT >>>
...
def train_func(config):
    class VitCheXpert(L.LightningModule):
        def __init__(self, cfg, num_classes=14):
            ...
            # <<< RETRAIN SUPPORT >>>
            pretrained_uri = cfg.get("pretrained_model_uri", None)
            if pretrained_uri:
                # load entire LightningModule from MLflow
                self.model = mlflow.pytorch.load_model(pretrained_uri)
            else:
    ...
    # Ray checkpoint resume logic stays the same
...
config['pretrained_model_uri'] = f"runs:/{best_run.info.run_id}/model"
