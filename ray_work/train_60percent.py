...
def get_dataloaders(csv_path, batch_size):
    ...
    sixty_percent_len = int(0.6 * total_len)
    dataset_60 = torch.utils.data.Subset(full_dataset, list(range(sixty_percent_len)))

    # Split that into train and val (e.g., 80% train, 20% val of 60%)
    val_size = int(0.2 * sixty_percent_len)
    train_size = sixty_percent_len - val_size

    train_dataset, val_dataset = torch.utils.data.random_split(dataset_60, [train_size, val_size])
